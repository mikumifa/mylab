...
def build_parser() -> argparse.ArgumentParser:
    ...
    flow_cmd.add_argument("--run-dir", type=Path, help="Existing run directory to resume.")
    flow_cmd.add_argument("--repo", type=Path, help="Git-tracked experiment repository path for a new run.")
    flow_group = flow_cmd.add_mutually_exclusive_group(required=False)
    flow_group.add_argument("--goal", help="Plain-text experiment goal, or a file path whose contents should be used as the goal.")
    flow_group.add_argument("--lab-md", type=Path, help="Markdown file containing the experiment goal/spec.")
    flow_cmd.add_argument("--source-branch", help="Base Git branch for later experiment work. Defaults to current branch.")
    flow_cmd.add_argument("--run-id", help="Explicit run identifier for a new run. Defaults to a timestamp-based value.")
    flow_cmd.add_argument("--model", help="Optional Codex model override. If omitted, Codex uses its configured default.")
    flow_cmd.add_argument("--limit", type=int, default=100, help="Maximum number of tasks to process in one call.")
    ...
    queue_iter_cmd.add_argument("--run-dir", required=True, type=Path, help="Existing run directory.")
    queue_iter_cmd.add_argument("--parent-plan", required=True, help="Parent plan id, for example plan-001.")
    queue_iter_cmd.add_argument("--feedback", required=True, help="Observed result or next-step instruction.")
    queue_iter_cmd.add_argument("--model", help="Optional Codex model override for the follow-up executor.")
    ...
    init_run_cmd.add_argument("--repo", required=True, type=Path, help="Git-tracked experiment repository path.")
    init_run_group = init_run_cmd.add_mutually_exclusive_group(required=True)
    init_run_group.add_argument("--goal", help="Plain-text experiment goal, or a file path whose contents should be used as the goal.")
    init_run_group.add_argument("--lab-md", type=Path, help="Markdown file containing the experiment goal/spec.")
    init_run_cmd.add_argument("--source-branch", help="Base Git branch for later experiment work. Defaults to current branch.")
    init_run_cmd.add_argument("--run-id", help="Explicit run identifier. Defaults to a timestamp-based value.")
    init_run_cmd.add_argument("--model", help="Optional Codex model override stored in the queued tasks.")
    ...
    poll_cmd.add_argument("--run-dir", required=True, type=Path, help="Run directory created by init-run.")
    poll_cmd.add_argument("--limit", type=int, default=3, help="Maximum number of queued tasks to process.")
    poll_cmd.add_argument("--allow-exec", action="store_true", help="Allow the run_executor stage to actually call codex.")
    ...
    create_cmd.add_argument("--run-dir", type=Path, help="Existing run directory. If omitted, a new run will be bootstrapped.")
    create_cmd.add_argument("--repo", type=Path, help="Repository path used when bootstrapping a new run.")
    create_group = create_cmd.add_mutually_exclusive_group(required=False)
    create_group.add_argument("--goal", help="Plain-text experiment goal, or a file path whose contents should be used for a new run.")
    create_group.add_argument("--lab-md", type=Path, help="Markdown file used for a new run.")
    create_cmd.add_argument("--source-branch", help="Base branch used when bootstrapping a new run.")
    create_cmd.add_argument("--run-id", help="Run identifier used when bootstrapping a new run.")
    ...
    iterate_cmd.add_argument("--run-dir", required=True, type=Path, help="Existing run directory.")
    iterate_cmd.add_argument("--feedback", required=True, help="Observed result or instruction for the next iteration.")
    iterate_cmd.add_argument("--parent-plan", required=True, help="Parent plan id, for example plan-001.")
    ...
    format_cmd.add_argument("--repo", type=Path, help="Repository path. Required when --run-dir is omitted.")
    format_cmd.add_argument("--run-dir", type=Path, help="Existing run directory. If set, repo is taken from the run manifest.")
    ...
    prepare_cmd.add_argument("--run-dir", required=True, type=Path, help="Existing run directory.")
    prepare_cmd.add_argument("--plan-id", help="Plan id. Defaults to manifest.latest_plan_id.")
    prepare_cmd.add_argument("--model", help="Optional Codex model override for the generated command.")
    ...
    run_cmd.add_argument("--run-dir", required=True, type=Path, help="Existing run directory.")
    run_cmd.add_argument("--plan-id", help="Plan id. Defaults to manifest.latest_plan_id.")
    run_cmd.add_argument("--model", help="Optional Codex model override for execution.")
    run_cmd.add_argument("--full-auto", action="store_true", help="Pass Codex full-auto mode through to execution.")
    ...
    summary_cmd.add_argument("--run-dir", required=True, type=Path, help="Existing run directory.")
    summary_cmd.add_argument("--plan-id", required=True, help="Plan id to summarize.")
    summary_cmd.add_argument("--status", required=True, help="Short status label, for example success or failed.")
    summary_cmd.add_argument("--outcome", required=True, help="Human-readable one-line outcome summary.")
    summary_cmd.add_argument("--evidence", nargs="+", required=True, help="One or more evidence paths or notes.")
    summary_cmd.add_argument("--artifacts", nargs="+", required=True, help="One or more produced artifact paths.")
    summary_cmd.add_argument("--next-iteration", nargs="+", required=True, help="One or more next-step items.")
    return parser
...
def cmd_run_flow(args: argparse.Namespace) -> int:
    if args.run_dir:
        ...
        if not args.repo or not (args.goal or args.lab_md):
            raise ValueError("run requires either --run-dir or (--repo and one of --goal/--lab-md)")
        repo_path = args.repo.expanduser().resolve()
        lab_md = args.lab_md.expanduser().resolve() if args.lab_md else None
        goal_text = lab_input_text(args.goal, lab_md)
        run_id = args.run_id or make_run_id(goal_text)
    ...
    outputs = SerialFlowRunner(run_dir, allow_exec=True).run_until_blocked(limit=args.limit)
    for item in outputs:
        print(f"{item['task_id']} {item['kind']} {item['output']}")
    return 0
