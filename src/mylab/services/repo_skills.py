...
Use this pattern when a prepared executor script already exists at `trials/trial-001/executor.sh` and you need to run a long training job under mylab monitoring.
...
mylab tool start-job \\
  ...
  --trial-id trial-001 \\
  --name train \\
  --command 'bash /abs/path/to/.mylab_runs/20260316_120000_example/trials/trial-001/executor.sh train'
```
